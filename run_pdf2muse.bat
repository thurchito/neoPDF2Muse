...
start /wait /b "" "zenity.exe" --list --title="neoPDF2Muse" --text="Select runtime version:" --radiolist --column="Check" --column="Options" "FALSE" "CPU" "FALSE" "GPU (ONNX Runtime)" "FALSE" "TensorFlow GPU" "FALSE" "Gradio UI" --width=250 --height=250 > "%TEMP%\selection.tmp"
...
if "%choice%"=="Gradio UI" (
    ...
    if errorlevel 1 (
        taskkill /f /im zenity.exe >nul & start /b "" "zenity.exe" --warning --title="neoPDF2Muse" --text="Gradio UI not found. Installing..." --width=200 --height=50
        call pip install gradio
    )
    python gradio_app.py
)
taskkill /f /im zenity.exe >nul & start /b /wait "" "zenity.exe" --info --title="neoPDF2Muse" --text="Processing completed. Be sure to check the open 'cmd' window for possible ignored errors." --width=300 --height=50
exit
...
:fileselect
    ...
    taskkill /f /im zenity.exe >nul && start /wait /b "" "zenity.exe" --file-selection --title="Select the desired PDF file for conversion:" > "%TEMP%\selection.tmp"
    ...
    start /wait /b "" "zenity.exe" --file-selection --directory --title="Select the desired output directory:" > "%TEMP%\selection.tmp"
